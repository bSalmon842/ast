...
function b32 DebugButton(Game_RenderCommands *commands, Game_Input *input, Camera camera, char *text, char *font, v2f textOrigin, f32 textScale, s32 zLayer, v4f textColour, v2f min, v2f max)
{
    b32 result = false;
    
    PushText(commands, camera, text, font, V3F(textOrigin, 0.0f),
             textScale, zLayer, textColour);
    
    v2f mousePos = V2F((f32)input->mouse.x, (f32)input->mouse.y);
    if (mousePos > min && mousePos < max)
    {
        PushRect(commands, camera, min, max, 0.0f, 0.0f, DEBUG_LAYER - 2, V4F(1.0f, 0.0f, 1.0f, 1.0f));
...
function void DebugDiscreteSlider(Game_RenderCommands *commands, Camera camera, Game_Input *input, f32 *a, f32 *b, f32 leftBound, f32 rightBound, f32 y, f32 height, s32 zLayer)
{
    ...
    PushRect(commands, camera, V2F(leftBound, y - 1.0f), V2F(rightBound, y + 1.0f), 0.0f, 0.0f, zLayer, V4F(1.0f));
    ...
    PushRect(commands, camera, leftMin, leftMax, 0.0f, 0.0f, zLayer + 1, V4F(1.0f, 0.0f, 0.0f, 1.0f));
    PushRect(commands, camera, rightMin, rightMax, 0.0f, 0.0f, zLayer + 1, V4F(0.0f, 0.0f, 1.0f, 1.0f));
    PushRect(commands, camera, middleMin, middleMax, 0.0f, 0.0f, zLayer + 1, V4F(0.0f, 1.0f, 0.0f, 1.0f));
}